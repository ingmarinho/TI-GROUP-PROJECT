...
PI_THREAD(openRightBarrier)                                             // Code om de rechter slagboomd dicht te zetten.
{
...
void openLeftBarrier()                                                  // Code om de linker slagboom dicht te zetten.
{
...
PI_THREAD(closeRightBarrier)                                            // Code om de rechter slagboom open te zetten.
{
    for (unsigned int i = 50; i < 100; i += 2)                          // Zorgt voor de stepsize van de servo voor een 'vloeiende beweging'.
    {
        changeServoPosition(SERV2, i);
...
void closeLeftBarrier()                                                 // Code om de rechter slagboom open te zetten.
{
    for (unsigned int i = 100; i > 50; i -= 2)                          // Zorgt voor de stepsize van de servo voor een 'vloeiende beweging'
    {
        changeServoPosition(SERV3, i);
...
PI_THREAD(activateTrafficLights)                                        // Thread voor de stoplichten op de brug.
{
    while (sequenceStart == 0)                                          // Stoplichten blijven aan totdat de "sequence" voorbij is.
    {
        activateLeds();                                                 // Stoplichten gaan knipperen.
        delay(200);
    }
    return 0;
}
...
int closeBarriers()                                                     // Code om beide slagebomen dicht te laten gaan.
{
    ...
    closeLeftBarrier();                                                 // Stuurt de linker slagboom aan.
...
int openBarriers()                                                      // Code om beide slagbomen te openen.
{
    ...
    openLeftBarrier();                                                  // Stuur de linker slagboom aan.
...
{
    while (true)                                                            // Main program loop die continu checkt of er een boot voor de brug staat.
    {
        int distanceFront = getCurrentDistance(TRIG1, ECHO1);
        int distanceBack = getCurrentDistance(TRIG2, ECHO2);
