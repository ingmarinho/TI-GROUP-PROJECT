...
SocketException::SocketException(const string &message) throw() : runtime_error(message)
{
}

SocketException::SocketException(const string &message, const string &detail) throw() : runtime_error(message + ": " + detail)
{
}
...
SocketTimedOutException::SocketTimedOutException(const string &message) throw() : SocketException(message)
{
}
...
static addrinfo *getAddressInfo(const char *host, const char *service,
                                SocketAddress::AddressType atype) noexcept(false)
{
...
SocketAddress::SocketAddress(const char *host, const char *service,
                             AddressType atype) noexcept(false)
{
...
string SocketAddress::getAddress() const noexcept(false)
{
    void *numericAddress;
...
in_port_t SocketAddress::getPort() const noexcept(false)
{
    switch (((sockaddr *)&addr)->sa_family)
    {
    case AF_INET:
...
SocketAddress Socket::getLocalAddress() noexcept(false)
{
...
size_t CommunicatingSocket::recv(void *buffer, int bufferLen) noexcept(false)
{
...
size_t CommunicatingSocket::recvFully(void *buffer, int bufferLen) noexcept(false)
{
    int rcount = 0;
...
SocketAddress CommunicatingSocket::getForeignAddress() noexcept(false)
{
    sockaddr_storage addr;
    socklen_t addrLen = sizeof(addr);
...
TCPSocket::TCPSocket(const char *foreignAddress,
                     in_port_t foreignPort) noexcept(false)
{
    myStream = NULL;
    sockDesc = -1;
...
void TCPSocket::bind(const SocketAddress &localAddress) noexcept(false)
{
...
void TCPSocket::connect(const SocketAddress &foreignAddress) noexcept(false)
{
    if (sockDesc < 0)
...
iostream &TCPSocket::getStream() noexcept(false)
{
    if (myStream == NULL)
    {
...
TCPServerSocket::TCPServerSocket(in_port_t localPort, int queueLen) noexcept(false)
{
    char buffer[1024];
...
void TCPServerSocket::bind(const SocketAddress &localAddress) noexcept(false)
{
    createSocket(localAddress, SOCK_STREAM, IPPROTO_TCP);
...
void UDPSocket::setBroadcast() noexcept(false)
{
    ...
    if (n < 0)
        throw SocketException("Broadcast allowance set failed (setsockopt())");
}

void UDPSocket::bind(const SocketAddress &localAddress) noexcept(false)
{
    createSocket(localAddress, SOCK_DGRAM, IPPROTO_UDP);
    int n = ::bind(sockDesc, localAddress.getSockaddr(), localAddress.getSockaddrLen());
    if (n < 0)
        throw SocketException("can not perform bind");
}

void UDPSocket::connect(const SocketAddress &foreignAddress) noexcept(false)
{
    if (sockDesc < 0)
...
void UDPSocket::setMulticastLoop(bool loop) noexcept(false)
{
...
//added
void UDPSocket::setTimeOut(int sec) noexcept(false)
{
